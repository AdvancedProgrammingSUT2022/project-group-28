...
public class Game {
    ...
    Game(ArrayList<User> users) {
        ...
        for (int i = 0; i < 100; i++) {
            for (int j = 0; j < 100; j++) {
                if (i%2 == 0 && j%0 == 0) this.map[i][j] = new Tile(Terrain.PLAIN, TerrainFeature.FOREST, null, null, null);
                else if (i%2 == 1 && j%2 == 1) this.map[i][j] = new Tile(Terrain.PLAIN, TerrainFeature.FOREST, null, null, null);
